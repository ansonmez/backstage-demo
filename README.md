...
Fork the following repo `https://github.com/ansonmez/backstage-demo` to your organization.
...
cd backstage-demo
find . -type f -not -path '*/\.git/*' -exec sed -i "s/ansonmez/${github_organization}/g" {} +
find . -type f -not -path '*/\.git/*' -exec sed -i "s/ansonmez/${github_organization}/g" {} +
find . -type f -not -path '*/\.git/*' -exec sed -i "s/s201.anillocal.com/${base_domain}/g" {} +
git add -A
...
Fork the following repo `https://github.com/ansonmez/shared-workflows` to your organization.
Then execute the following commands
...
Fork the following repo `https://github.com/ansonmez/software-templates` to your organization.
Then execute the following commands
...
Fork the following repo `https://github.com/ansonmez/backstage-app-demo` to your organization.
...
oc create secret generic gitlab-credentials -n backstage --from-literal=AUTH_GITHUB_CLIENT_ID=${backstage_gitlab_client_id} --from-literal=AUTH_GITLAB_CLIENT_SECRET=${backstage_gitlab_client_secret} --from-literal=GITLAB_TOKEN=${gitlab_org_admin_pat} --from-literal=GITLAB_ORG=${gitlab_organization}
